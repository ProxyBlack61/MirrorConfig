<template>
  ...
  <div class="container-fluid">
    <h1 class="display-4 text-center">{{ $t("weather.header") }}</h1>
    <basic-sub-heading textString="weather.GeneralHeader" />

    <div class="list-group mb-5 shadow">
      <basic-toggle
        textString="weather.Enabled"
      ...
      <basic-select
        textString="Position"
        v-model="weatherParent.module.position"
        :options="positionOptions"
        :disabled="weatherParent.module.disabled"
      />
    </div>

    <div class="row" style="padding-left: 15%; padding-right: 15%">
      <v-btn size="x-large" color="indigo" @click="saveWeatherSettings">
        {{ $t("buttonSave") }}
      </v-btn>
    </div>
...
export default {
  ...
  created() {
    fetch("http://127.0.0.1:3000/weatherSettings", {
      method: "GET",
      mode: "cors",
      headers: {
      ...
      .then((data) => {
        if (Object.keys(data.module).length !== 0) {
          this.weatherParent.module = data.module;
        }
      });
  },
  methods: {
    saveWeatherSettings() {
      fetch("http://127.0.0.1:3000/weatherSettings", {
        ...
        body: JSON.stringify(this.weatherParent),
      }).then((response) => {
        if (response.ok) {
          this.snackbar = true;
        }
  ...
  computed: {
    moduleEnabled: {
      get() {
        return !this.weatherParent.module.disabled;
      },
      set(value) {
        this.weatherParent.module.disabled = !value;
      },
    },
  },
};
